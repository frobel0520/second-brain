...
`ask`/`translate` 需要設定環境變數 `ANTHROPIC_API_KEY` 才能呼叫 Anthropic API;沒設定的話指令會直接印出提示並結束,不會噴出未處理的例外。`add`/`add-feed`/`feeds` 系列指令**不**需要這個 key——會自動嘗試翻譯新加入的文件,失敗(通常是沒設 key)就靜默跳過,不影響其他 ingestion 流程。

所有時間戳記(`created_at`/`last_synced_at`)在資料庫裡都是 UTC,顯示給人看的時候會自動轉成 UTC+8(`config.DISPLAY_TIMEZONE`)。
...
├── processing/       # 清洗、切塊、embedding、自動標籤、翻譯
...
- **翻譯是「另存一欄,原文不動」,不是取代**:`Document.translated_content`(對應 SQLite `documents.translated_content` 欄位)存繁體中文翻譯,`Document.content` 保持原文不變。切塊(`chunk_document()`)、embedding、`search`/`ask` 的語意搜尋全部只作用在原文,翻譯純粹是給人讀的,不影響既有搜尋品質。`add`/`add-feed`/`feeds` 系列指令會在 `ingest_document()` 裡自動嘗試翻譯(`_translate_best_effort()`),失敗(常見原因是沒設 `ANTHROPIC_API_KEY`)就靜默把 `translated_content` 留空,**不會**擋住 ingestion 本身——這幾個指令本來就不需要 Anthropic API key 也能動,自動翻譯是額外加值,不該變成新的必要條件。`second-brain translate` 指令則相反:是使用者主動要求翻譯,遇到認證失敗會直接停止並清楚回報(不會靜默略過),避免對幾十篇文件重複噴出同一個錯誤
- **`documents` 表的 `translated_content` 欄位用 `ALTER TABLE` 遷移既有資料庫**,不是要求使用者砍掉重建:`sqlite_store._connect()` 每次連線都會呼叫 `_ensure_translated_content_column()`,用 `PRAGMA table_info` 檢查欄位存不存在,不存在才補一次 `ALTER TABLE ADD COLUMN`。這是這個專案第一次用真的 schema migration 處理欄位新增(先前 `tags`/`feeds` 表都是要求刪掉空資料庫重建),之後如果還要加欄位,可以照這個模式做,不用再讓使用者手動處理
