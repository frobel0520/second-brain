...
最後更新:2026-07-13(第十六輪,瀏覽/搜尋頁面排序與版面調整,已 commit `12da665`)
...
- **16**(2026-07-13,commit `12da665`):瀏覽/搜尋頁面,只動 `web.py`。瀏覽依加入時間
  由新到舊、一頁 10 筆左右兩欄各 5、箭頭式分頁、移除「片段數」、刪除鍵縮成小「×」;
  搜尋加排序下拉(相關性/日期新到舊)。**修掉一個埋著的問題:搜尋結果原本被寫死永遠依
...
- **瀏覽**:文件卡片(標題/時間/分類/標籤/來源 + 右上小「×」刪除鈕)。第 16 輪:依加入時間新到舊、
